...
add_filter( 'woocommerce_available_payment_gateways', 'depends_payments_on_shipping' );
 
function depends_payments_on_shipping( $available_gateways ) { //echo 'paym_ship';
	...
	$chosen_methods = WC()->session->get( 'chosen_shipping_methods' ); //var_dump($available_gateways[ 'cod' ]);
 
	//echo '<pre>';print_r( $chosen_methods );
 /**/
	if ( isset( $available_gateways[ 'cod' ] ) && 'flat_rate:1' == $chosen_methods[0] ) { //echo 'is isset';
		unset( $available_gateways[ 'cod' ] ); // отключаем оплату при доставке наличным
		unset( $available_gateways[ 'cod1' ] ); // отключаем оплату при доставке картой
	}
/**/ 
	return $available_gateways;
...
//add_action('woocommerce_checkout_update_order_review', 'on_action_cart_updated');
